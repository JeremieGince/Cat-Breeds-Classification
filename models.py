from tensorflow.keras.layers import Conv2D, Input, Reshape, RepeatVector, concatenate, UpSampling2D, Flatten, Conv2DTranspose
...
def generate_model(lr=1e-3, img_size=128):
    ...
    batch, height, width, channels = Kbackend.int_shape(encoder)

    features_average_ip = tf.keras.layers.GlobalAveragePooling2D()

    resnet_features = tf.keras.layers.Dense(1000)(features_average_ip)
    fusion = RepeatVector(height * width)(resnet_features)
    ...
    model = Model([encoder_ip, features_average_ip], decoder, name='Colorizer')
